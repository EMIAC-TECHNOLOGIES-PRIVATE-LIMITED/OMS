...
async function main() {
  try {
    ...
    const permissionsData = [
      { key: 'site', description: 'Permission to view Sites route' },
      { key: 'vendor', description: 'Permission to view Vendors route' },
      { key: 'client', description: 'Permission to view Client route' },
      { key: 'order', description: 'Permission to view Orders route' },
    ];
    ...
    const sitePermission = permissions.find((p) => p.key === 'site');
    const vendorPermission = permissions.find((p) => p.key === 'vendor');
    const clientPermissions = permissions.find((p) => p.key === 'client');
    const orderPermission = permissions.find((p) => p.key === 'order');
